...
def inter_day_change(date_1):
    ...
    logging.info(
        f"{date_0} to {date_1} {rates[date_1]:0.5f}-{rates[date_0]:0.5f} = {change:0.5f}"
    )
    return change
...
def twitter_api():
    ...
    auth.set_access_token(
        key=config["TWITTER_ACCESS_KEY"], secret=config["TWITTER_ACCESS_SECRET"]
    )
...
def search_stories():
    ...
    for story in stories:
        if (
            "Market data: FTSE 100, Pound/\nDollar, Pound/Euro, US markets, Oil prices."
            in story["snippet"]
        ):
